...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_product'])) {
    ...
    // ADDED: Get short description from form
    ...
    if (empty($name) || $price === false || $cost_price === false || $category_id === false || $stock === null) {
        ...
        try {
            ...
            if ($image_name !== false) {
                // ADDED: 'short_description' to SQL insert
                ...
                // ADDED: $short_description to execute array
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_product'])) {
    ...
    if (!$product_id || empty($name) || $price === false || $cost_price === false || $category_id === false || $stock === null) {
        ...
        try {
            ...
            // ADDED: 'short_description = ?' to SQL update
                            ...
                            <td class="text-end">
                                ...
                                <?php else: ?>
                                    ...
                                    <button type="button" class="btn btn-sm btn-outline-danger delete-product-btn"
                                        data-id="<?= htmlspecialchars($prod['id']) ?>"><i class="bi bi-trash"></i> DELETE
                                    </button>
                                <?php endif; ?>
                            </td>
                        </tr>
        ...
        <div class="modal-content">
            <form method="post" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    <div class="row g-3">
                        <div class="col-md-6"><label for="add_product_name" class="form-label">Product Name</label><input type="text" name="name" id="add_product_name" required class="form-control"></div>
                        <div class="col-md-6"><label for="add_product_category" class="form-label">Category</label><select name="category_id" id="add_product_category" required class="form-select">
                                <option value="" disabled selected>-- Select --</option><?php foreach ($categories as $cat): ?><option value="<?= esc_html($cat['id']) ?>"><?= esc_html($cat['name']) ?></option><?php endforeach; ?>
                            </select></div>

                        <div class="col-12">
                            <label for="add_product_short_description" class="form-label">Short Description</label>
                            <input type="text" name="short_description" id="add_product_short_description" class="form-control" maxlength="255">
                            <small class="form-text text-muted">A brief, one-line summary of the product.</small>
                        </div>

                        <div class="col-12"><label for="add_product_description" class="form-label">Full Description</label><textarea name="description" id="add_product_description" required class="form-control" rows="3"></textarea></div>
                        <div class="col-md-4"><label for="add_product_cost_price" class="form-label">Cost Price</label>
                            <div class="input-group"><span class="input-group-text">৳</span><input type="number" name="cost_price" id="add_product_cost_price" required class="form-control" step="0.01" min="0"></div>
                        </div>
                        <div class="col-md-4"><label for="add_product_price" class="form-label">Selling Price</label>
                            <div class="input-group"><span class="input-group-text">৳</span><input type="number" name="price" id="add_product_price" required class="form-control" step="0.01" min="0"></div>
                        </div>
                        <div class="col-md-4"><label for="add_product_stock" class="form-label">Stock</label><input type="number" name="stock" id="add_product_stock" required class="form-control" min="0"></div>
                        <div class="col-md-6"><label for="add_product_image" class="form-label">Main Image</label><input type="file" name="image" id="add_product_image" class="form-control" required><small class="form-text text-muted">This is the featured image.</small></div>
                        <div class="col-md-6"><label for="add_additional_images" class="form-label">Additional Images</label><input type="file" name="additional_images[]" id="add_additional_images" class="form-control" multiple></div>
                    </div>
                </div>
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form method="post" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    <div class="row g-3">
                        <div class="col-md-6"><label for="edit_product_name" class="form-label">Product Name</label><input type="text" name="name" id="edit_product_name" required class="form-control"></div>
                        <div class="col-md-6"><label for="edit_product_category" class="form-label">Category</label><select name="category_id" id="edit_product_category" required class="form-select"><?php foreach ($categories as $cat): ?><option value="<?= esc_html($cat['id']) ?>"><?= esc_html($cat['name']) ?></option><?php endforeach; ?></select></div>

                        <div class="col-12">
                            <label for="edit_product_short_description" class="form-label">Short Description</label>
                            <input type="text" name="short_description" id="edit_product_short_description" class="form-control" maxlength="255">
                        </div>

                        <div class="col-12"><label for="edit_product_description" class="form-label">Full Description</label><textarea name="description" id="edit_product_description" required class="form-control" rows="3"></textarea></div>
                        <div class="col-md-4"><label for="edit_product_cost_price" class="form-label">Cost Price</label>
                            <div class="input-group"><span class="input-group-text">৳</span><input type="number" name="cost_price" id="edit_product_cost_price" required class="form-control" step="0.01" min="0"></div>
                        </div>
                        <div class="col-md-4"><label for="edit_product_price" class="form-label">Selling Price</label>
                            <div class="input-group"><span class="input-group-text">৳</span><input type="number" name="price" id="edit_product_price" required class="form-control" step="0.01" min="0"></div>
                        </div>
                        <div class="col-md-4"><label for="edit_product_stock" class="form-label">Set Stock</label><input type="number" name="stock" id="edit_product_stock" required class="form-control" min="0"></div>
                        <div class="col-md-6"><label for="edit_product_image" class="form-label">Change Main Image</label><input type="file" name="image" id="edit_product_image" class="form-control"><small class="form-text text-muted">Leave blank to keep current image.</small></div>
                        <div class="col-md-6"><label for="edit_additional_images" class="form-label">Add More Images</label><input type="file" name="additional_images[]" id="edit_additional_images" class="form-control" multiple></div>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (editProductModal) {
            editProductModal.addEventListener('show.bs.modal', function(event) {
                ...
                // ADDED: Get short description from data attribute
                ...
                // ADDED: Populate short description field
        ...
        // ...The rest of your original JavaScript code for modals and AJAX calls...
